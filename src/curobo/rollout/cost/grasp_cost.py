...
class GraspCost(CostBase, GraspCostConfig):
    def __init__(self, config: GraspCostConfig, grasp_energy: GraspEnergyBase = None):
        ...
        self.new_contact_stage = self.contact_stage = 0
        ...
        self.contact_normals = self.tensor_args.to_device(self.contact_strategy["contact_normals"])  # (n_contact, 3)
        ...
        if task["f"] is not None:
            ...
            if robust_angle > 0:
                if robust_angle > 90:
                    ...
                    robust_angle = (
                        90 / 180 * np.pi
                    )  # treat >90 angle similarly as 180. use 90 to compute the below directions.
                else:
    ...
    #     self.switch_stage_flag = switch_stage_flag = self.contact_stage < new_contact_stage
    #     save_qpos_flag = switch_stage_flag and self.contact_strategy["save_qpos"][self.contact_stage]

    #     if self.contact_strategy["ge_stage_next_first"]:
    #         ge_stage_flag = (self.contact_stage <= max_ge_stage) or (new_contact_stage <= max_ge_stage)
    #     else:
    #         ge_stage_flag = new_contact_stage <= max_ge_stage

    #     # After updating self.contact_stage
    ...
    def _get_contact_stage(self, opt_progress):
        ...
        self.switch_stage_flag_this_iter = self.contact_stage < self.new_contact_stage  # with swtich at this iteration

        self.contact_stage = self.new_contact_stage  # contact stage for this iter
        contact_query_mode = self.contact_strategy["contact_query_mode"][self.contact_stage]
        contact_distance = self.contact_strategy["distance"][self.contact_stage]
        if self.contact_query_mode != contact_query_mode:
            self.contact_query_mode = contact_query_mode
        ...
        max_ge_stage = self.contact_strategy["max_ge_stage"]
        new_contact_stage = (
            self.contact_strategy["opt_progress"] <= opt_progress
        ).int().sum() - 1  # stage of the next iteration
        self.switch_stage_flag = switch_stage_flag = (
            self.contact_stage < new_contact_stage
        )  # will switch at the next iteration
        ge_stage_flag = (self.contact_stage <= max_ge_stage) or (new_contact_stage <= max_ge_stage)
        save_qpos_flag = switch_stage_flag and self.contact_strategy["save_qpos"][self.contact_stage]

        self.new_contact_stage = new_contact_stage

        # print(f"ge_stage_flag: {ge_stage_flag}, ge_stage_flag_2: {ge_stage_flag_2}")
        # print(f"opt_progress: {opt_progress}, save_qpos_flag: {save_qpos_flag}")
    ...
    def loss_w_ge(self, pos, dist, normal, contact_distance, opt_progress, norminal_normals):
        grasp_energy, grasp_error, contact_frame, contact_force = self.GraspEnergy.forward(
        ...
        E_dist = self.weight[self.contact_stage][1] * ((dist - contact_distance) ** 2).mean(dim=-1)
        E_regu = 0 * E_dist
        ...
        return (
            ...
            dist_error,
            grasp_error.detach(),
            contact_frame.detach(),
            contact_force.detach(),
        )

    def loss_wo_ge(self, raw_spheres, contact_distance, mesh_contacts, mesh_dist, opt_progress, norminal_normals):
        """
        Loss with fixed target contact locations on the object.
        Args:
            mesh_contacts: position of closest points on the robot mesh.
        """

        if mesh_dist is None:  # use sphere collision
            target_sphere_center = self.target_contact_position - self.target_contact_normal * (
                raw_spheres[..., -1] + contact_distance
            ).unsqueeze(-1)
            E_dist = self.weight[self.contact_stage][1] * ((raw_spheres[..., :3] - target_sphere_center) ** 2).sum(
                dim=-1
            ).mean(dim=-1)  # distance between robot and object
            E_regu = 0 * E_dist
            dist_error = (raw_spheres[..., :3] - target_sphere_center).norm(dim=-1).max(dim=-1)[0]
        else:  # use mesh collision
            E_dist = self.weight[self.contact_stage][1] * (
                ((mesh_dist - contact_distance) ** 2).mean(dim=-1)
            )  # distance between robot and object
            target_mesh_contacts = self.target_contact_position - self.target_contact_normal * contact_distance
            E_regu = self.weight[self.contact_stage][2] * ((mesh_contacts - target_mesh_contacts) ** 2).sum(
                dim=-1
            ).mean(dim=-1)  # regularization to keep the mesh contacts on the robot close to the target contacts.
            dist_error = (mesh_dist - contact_distance).abs().max(dim=-1)[0]

        E_angle = 0 * E_dist  # grasp energy
        E_normal = -self.weight[self.contact_stage][3] * (
            -1.0 + (self.target_contact_normal * norminal_normals).sum(dim=-1)
        ).mean(
            dim=-1
        )  # loss between the target contact normals on the object  and the norminal normals of the fingertips
        ...
        # E_dist = (
        #     self.weight[self.contact_stage][1]
        ...
        #     E_dist += self.weight[self.contact_stage][1] * ((mesh_contacts - target_sphere_center) ** 2).sum(
        #         dim=-1
        #     ).mean(dim=-1)
        #     E_regu = self.weight[self.contact_stage][2] * (((mesh_dist - contact_distance) ** 2).mean(dim=-1))
        #     dist_error = (mesh_dist - contact_distance).abs().max(dim=-1)[0]
        # else:
        #     target_sphere_center = self.target_contact_position - self.target_contact_normal * (
        #         raw_spheres[..., -1] + contact_distance
        ...
        return E_angle, E_normal, E_dist, E_regu, dist_error
    ...
    def forward(self, robot_sphere_in, link_pos_quat, env_query_idx, opt_progress):
        ...
        # To compute normal cost
        query_info = link_pos_quat[..., self.contact_mesh_idx, :]
        rot = torch_quaternion_to_matrix(query_info[..., 3:])
        norminal_normals_local = (
            self.contact_normals.unsqueeze(0).unsqueeze(0).repeat(link_pos_quat.shape[0], 1, 1, 1).unsqueeze(-1)
        )  # (b, 1, n_contact, 3, 1)
        norminal_normals_global = (rot @ norminal_normals_local).squeeze(-1)  # (b, 1, n_contact, 3)
        ...
        if self.contact_query_mode == -1:
            ...
            if self.count % 5 == 0:
                ...
                raw_pos_two, raw_dist, self.raw_normal, self.debug_pos, self.debug_normal = (
                    self.world_coll_checker.get_mesh_contact_pdn(
                        query_info.detach(), perturb, env_query_idx, dist_upper_bound
                    )
                )
                self.raw_normal = (
                    -self.raw_normal
                )  # flip normal direction to point inwards the object, inconsisent with sphere query
                self.raw_pos_robot_frame = (
                    (rot.transpose(-1, -2) @ (raw_pos_two[..., 3:] - query_info[..., :3]).unsqueeze(-1))
                    .squeeze(-1)
                    .detach()
                )
        ...
        if not ge_stage_flag:
            E_angle, E_normal, E_dist, E_regu, dist_error = self.loss_wo_ge(
                robot_contact_points,
                contact_distance,
                raw_pos_robot_with_grad,
                mesh_dist_with_grad,
                opt_progress,
                norminal_normals_global,
            )
            grasp_error = contact_frame = contact_force = None
        else:
            E_angle, E_normal, E_dist, E_regu, dist_error, grasp_error, contact_frame, contact_force = self.loss_w_ge(
                raw_pos,
                raw_dist,
                raw_normal,
                contact_distance,
                opt_progress,
                norminal_normals_global,  # new added
            )

        if ge_stage_flag and switch_stage_flag:  # this iter involves grasp energy and will switch stage at next iter
        ...
        debug = {
            ...
            "contact_stage": self.tensor_args.to_device(self.contact_stage).repeat(dist_error.shape[0]),  # [b]
        }
