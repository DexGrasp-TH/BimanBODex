...
class QPEnergy(GraspEnergyBase):
    ...
    def _init_LCQP_u(self, batch, num_points):
        """
        Mingrui: Not modified to support upper bound of the constraints.
        """
        ...
        # Constraints: Ax <= B
        G_matrix = self.tensor_args.to_device(
            torch.zeros(
                (
                    batch,
                    (self.num_friction_approx + 2) * num_points + 1 + len(self.pressure_constraints),
                    num_f_strength + 1,
                )
            )
        )
        h_matrix = self.tensor_args.to_device(
            torch.zeros((batch, (self.num_friction_approx + 2) * num_points + 1 + len(self.pressure_constraints)))
        )
        ...
        aranged_angles = self.tensor_args.to_device(
            torch.arange(self.num_friction_approx) * 2 * np.pi / self.num_friction_approx
        ).unsqueeze(-1)
    ...
    def _init_LCQP_lu(self, batch, num_points):
        ...
        G_matrix = self.tensor_args.to_device(
            torch.zeros(
                (
                    batch,
                    (self.num_friction_approx + 1) * num_points + 1 + len(self.pressure_constraints),
                    num_f_strength + 1,
                )
            )
        )
        l_matrix = self.tensor_args.to_device(
            torch.zeros((batch, (self.num_friction_approx + 1) * num_points + 1 + len(self.pressure_constraints)))
            - torch.inf
        )
        h_matrix = self.tensor_args.to_device(
            torch.zeros((batch, (self.num_friction_approx + 1) * num_points + 1 + len(self.pressure_constraints)))
        ...
        for i, constraint in enumerate(self.pressure_constraints):
            ...
            l_matrix[:, A_end2 + 1 + i] = constraint[1]  # lower bound
            h_matrix[:, A_end2 + 1 + i] = constraint[2]  # upper bound
    ...
    def _init_LCQP_lu_soft(self, batch, num_points):
        ...
        G_matrix = self.tensor_args.to_device(
            torch.zeros(
                (
                    batch,
                    (2 * self.num_friction_approx + 1) * num_points + 1 + len(self.pressure_constraints),
                    num_f_strength + 1,
                )
            )
        )
        l_matrix = self.tensor_args.to_device(
            torch.zeros((batch, (2 * self.num_friction_approx + 1) * num_points + 1 + len(self.pressure_constraints)))
            - torch.inf
        )
        h_matrix = self.tensor_args.to_device(
            torch.zeros((batch, (2 * self.num_friction_approx + 1) * num_points + 1 + len(self.pressure_constraints)))
    ...
    def construct_Q_matrix(self, grasp_matrix, target_wrenches):
        ...
        grasp_matrix = grasp_matrix.transpose(-3, -2).reshape(
            batch_num, 6, -1
    ...
    def forward(self, pos, normal, target_wrenches):
        ...
        grasp_energy = (
            (self.solution.unsqueeze(-2) @ Q_matrix2 @ self.solution.unsqueeze(-1)).view(batch, -1).clamp(min=0.0)
        )
