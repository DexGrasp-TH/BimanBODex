...
# Previous in-place multiply version
# @torch.jit.script
...
def _RELU(
    ...
    # .squeeze(-1) handles the 1D output per batch element
    out = torch.bmm(W[rho_ind, help_ind], input.unsqueeze(-1)).squeeze(-1)
    # In-place clamping of the specific 'z_part' range
    out[:, nx : nx + nc] = out[:, nx : nx + nc].clamp(l, u)
...
def compute_residuals(H, A, x, z, lam, rho, rho_min: float, rho_max: float):
    ...
    numerator = torch.div(
        primal_res,
        torch.max(
            torch.linalg.vector_norm(t1, dim=-1, ord=torch.inf), torch.linalg.vector_norm(z, dim=-1, ord=torch.inf)
        ),
    )
    denom = torch.div(
        dual_res,
        torch.max(
            torch.linalg.vector_norm(t2, dim=-1, ord=torch.inf), torch.linalg.vector_norm(t3, dim=-1, ord=torch.inf)
        ).clamp(min=0),
    )
...
class _RELUQP:
    ...
    def setup_rhos(self, nc, batch):
        ...
        self.rhos_matrix = self.rhos.view(-1, 1, 1, 1) * self.tensor_args.to_device(torch.eye(nc)).unsqueeze(
            0
        ).unsqueeze(1)  # [r, 1, nc, nc]
        self.rhos_inv_matrix = (1 / self.rhos.view(-1, 1, 1, 1)) * self.tensor_args.to_device(torch.eye(nc)).unsqueeze(
            0
        ).unsqueeze(1)  # [r, 1, nc, nc]
        return
    ...
    def solve(self, H, A, l, u):
        ...
        ArA = torch.matmul(
            A.transpose(-1, -2), rhosA, out=self.W_ks[1:, :, self.nx : 2 * self.nx, self.nx : 2 * self.nx]
        )  # TMP! [r, b, nx, nx]
        assert self.nx < self.nc
        nK_inv = torch.add(
            self.sigma + H, ArA, out=self.W_ks[1:, :, self.nx : 2 * self.nx, 2 * self.nx : 3 * self.nx]
        )  # TMP! [r, b, nx, nx]
        sig_ArA = torch.sub(
            self.sigma, ArA, out=self.W_ks[1:, :, self.nx : 2 * self.nx, self.nx : 2 * self.nx]
        ...
        for k in range(1, self.max_iter + 1):
            ...
            if k % self.check_interval == 0:
                x, z, lam = (
                    self.output[:, : self.nx],
                    self.output[:, self.nx : self.nx + self.nc],
                    self.output[:, self.nx + self.nc : self.nx + 2 * self.nc],
                )
                primal_res, dual_res, rho = compute_residuals(
                    H.squeeze(0), A.squeeze(0), x, z, lam, rho, self.rho_min, self.rho_max
                )
