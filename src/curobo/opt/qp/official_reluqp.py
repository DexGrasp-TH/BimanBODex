...
class OFFICIAL_RELUQP(QPSolver):
    ...
    def solve(self, Q_matrix, semi_Q_matrix, solution=None):
        ...
        for i in range(Q_matrix.shape[0]):
            self.solver.setup(
                H=Q_matrix[i], g=self.p_matrix, A=self.G_matrix[i], l=self.l_matrix[i], u=self.h_matrix[i]
            )
...
class ReLU_Layer(torch.nn.Module):
    ...
    def forward(self, input, idx):
        input = self.jit_forward(
            input, self.W_ks[idx], self.b_ks[idx], self.QP.l, self.QP.u, self.clamp_inds[0], self.clamp_inds[1]
        )
        return input
...
class ReLU_QP(object):
    ...
    def setup(
        ...
        self.output = (
            torch.cat([self.x, self.z, self.lam])
            .to(device=self.settings.device, dtype=self.settings.precision)
            .contiguous()
        )
    ...
    def update(self, g=None, l=None, u=None, Hx=None, Ax=None):
        """
    ...
    def solve(self):
        ...
        for k in range(1, stng.max_iter + 1):
            ...
            if k % stng.check_interval == 0 and stng.adaptive_rho:
                self.x, self.z, self.lam = (
                    self.output[:nx],
                    self.output[nx : nx + nc],
                    self.output[nx + nc : nx + 2 * nc],
                )
                primal_res, dual_res, rho = self.compute_residuals(
                    self.QP.H, self.QP.A, self.QP.g, self.x, self.z, self.lam, rho, stng.rho_min, stng.rho_max
                )

                if (
                    rho > self.layers.rhos[self.rho_ind] * stng.adaptive_rho_tolerance
                    and self.rho_ind < len(self.layers.rhos) - 1
                ):
                    self.rho_ind += 1
                ...
                if primal_res < stng.eps_abs * np.sqrt(nc) and dual_res < stng.eps_abs * np.sqrt(nx):
                    self.update_results(iter=k, status="solved", pri_res=primal_res, dua_res=dual_res, rho_estimate=rho)
        ...
        self.update_results(
            iter=stng.max_iter, status="max_iters_reached", pri_res=primal_res, dua_res=dual_res, rho_estimate=rho
        )
        return self.results

    def warm_start(
        self,
        x: torch.tensor or np.ndarray = None,
    ...
    def update_results(self, iter=None, status=None, pri_res=None, dua_res=None, rho_estimate=None):
        """
    ...
    def compute_residuals(H, A, g, x, z, lam, rho, rho_min: float, rho_max: float):
        ...
        numerator = torch.div(
            primal_res,
            torch.max(torch.linalg.vector_norm(t1, ord=torch.inf), torch.linalg.vector_norm(z, ord=torch.inf)),
        )
        denom = torch.div(
            dual_res,
            torch.max(
